 ...
 * $Id: theme.mdl.php 9343 2015-03-24 07:07:00Z youyi $
 */
...
{   
    ...
    public function install($ident)
    {
        ...
        if(!$data = $this->parse_xml($xml)){
            $this->msgbox->add(L('模板不存在或模板配置文件不正确'), 231);
        }else if($theme = $this->theme($ident)){
            $this->msgbox->add(L('该模板已经安装'), 232);
        }else{
            $a = array('theme'=>$ident);
            $a['title'] = $data['title'];
            $a['thumb'] = $data['thumb'];
    ...
    public function tmplsave($admin, $content, $tmpl)
    {
        ...
        if(!file_exists($file)){
            $this->msgbox->add(L('模板文件不存在'), 211);
            return false;
        }else if(!is_writable($file)){
            $this->msgbox->add(L('模板没有权限写入'), 212);
            return false;
        }else if(!$bak = file_get_contents($file)){
            $this->msgbox->add(L('文件读取失败'), 213);
            return false;
        }else if(md5($bak) == md5($content)){
            $this->msgbox->add(L('文件相同，未被修改'), 214);
            return false;
        }else{
