...
{   
    ...
    protected $_cols = 'menu_id,title,parent_id,shop_id,wx_sid,type,reply_id,content,link,orderby';
    ...
    public function items_by_weixin($shop_id)
    {
        $menu_list = array();
        if($items = $this->items(array('shop_id'=>$shop_id), array('parent_id'=>'ASC', 'orderby'=>'ASC'), 1, 30)){
    ...
    public function wechat_buttons($shop_id)
    {
        $buttons = array();
        if($items = $this->items_by_weixin($shop_id)){
            foreach($items as $k=>$v){
