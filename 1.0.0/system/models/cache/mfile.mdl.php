...
if(!defined("__CORE_DIR")){	exit("Access Denied");}if({time}===0 || __TIME<{time}){return {data};}return false;';
		...
		file_put_contents($this->cache_dir."cache_{$hash}.{$key}.php", $content);
	}

	public function get($key)
	{
		$fun = 'cache_'.md5($key);
		$file = $this->cache_dir.'cache_'.md5($key).".{$key}.php";
		if(file_exists($file)){
			return @include($file);
		}
		return false;
	...
	{
		K::M('io/file')->remove($this->cache_dir.'cache_'.md5($key).".{$key}.php");
	}
