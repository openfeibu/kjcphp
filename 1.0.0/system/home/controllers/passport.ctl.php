...
class Ctl_Passport extends Ctl
{
    ...
    {
        if($url = $this->GP('wxrebackurl')){
            $this->cookie->set('wxrebackurl', $url);
        }
        if($openid = $this->access_openid()){
            if($wxrebackurl = $this->cookie->get('wxrebackurl')){
                $this->cookie->set('wxrebackurl', '');
            }else{
                $wxrebackurl = $this->mklink('index');
            }
            header("Location:".$wxrebackurl);
            exit();
        }else{
            $this->msgbox->add('access token error', 211);
        }
    }
