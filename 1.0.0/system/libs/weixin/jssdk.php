...
class WeixinJSSDK {
    ...
    private function getJsApiTicket() {
        ...
        if (!$data = K::M('cache/cache')->get('wx_jsapi_ticket'.$this->appId)) {
            ...
            K::M('cache/cache')->set('wx_jsapi_ticket'.$this->appId, $data, 7000);
            }
        } else {
  ...
  private function getAccessToken() {
    ...
    if (!$data = K::M('cache/cache')->get('wx_access_token_'.$this->appId)) {
      ...
      if ($access_token) {
        ...
        K::M('cache/cache')->set('wx_access_token_'.$this->appId, $data, 7000);
      }
    } else {
  ...
  private function httpGet($url) {
    ...
    {
        // jsapi_ticket 应该全局存储与更新，以下代码以写入到文件中做示例    ;
        if (!$data = K::M('cache/cache')->get('wx_card_ticket'.$this->appId)) {
            $accessToken = $this->getAccessToken();
            ...
            if ($ticket) {
                ...
                K::M('cache/cache')->set('wx_card_ticket'.$this->appId, $data, 7000);
            }
        } else {
