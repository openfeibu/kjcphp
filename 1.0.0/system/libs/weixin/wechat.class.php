...
{   
    ...
    public function replyId($reply_id)
    {
        if($reply = K::M('weixin/reply')->detail($reply_id)){
            $article = K::M('weixin/reply')->format_wechat($reply);
            $this->replyNews(array($article));
        }
    }
    ...
    private function _replyData($msg, $msgType)
    {
        ...
        $str = $xml->asXML();               
        // 记录日志
        K::M('weixin/log')->log($this->data['ToUserName'], $str, '_replyData');
        echo ( $str );
    }
    /* 组装xml数据 */
...
class WeChatClient {
    ...
    public $weixin_type = 0; //0:订阅号, 1:服务号
    ...
    public function getUserInfoById($uid, $lang = '') {
        ...
        $res = json_decode(self::get($url), true);
        var_dump($this);echo 'FILE:',__FILE__,'LINE:',__LINE__;exit();
   ...
   /*
        $params = array('title'=>'这是一个标题', 'items'=>array('节点1', '节点2','节点3'，.....), 'remark'=>'这是一个描述');
    */
    public function sendTempMsg($openid, $template_id, $url, $params)
    {   
        $access_token = $this->getAccessToken();        
        $api_url = self::$_URL_API_ROOT . "/cgi-bin/message/template/send?access_token=$access_token";
        $data = array('touser'=>$openid, 'template_id'=>$template_id, 'url'=>$url,'topcolor'=>'#02970');
        $title = $params['title'] ? $params['title'] : '';
        $data['data']['first'] = array('value'=>urlencode($title), 'color'=>'#080808');        
        foreach((array)$params['items'] as $k=>$v){
            if(is_array($v)){
                $data['data'][$k] = $v;
            }else{
        ...
        $remark = $params['remark'] ? $params['remark'] : '点击【详情】查看详细信息';
        $data['data']['remark'] = array('value'=>urlencode($remark), 'color'=>'#080808');
