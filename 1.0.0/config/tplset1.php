<?php return array (
  ...
  'shopphonetpl' => '【筷叫餐】{$orderinfo[\'buyername\']}订单总价：{$orderinfo[\'allcost\']}元{if $orderinfo[\'paystatus\'] == 0}(未付){else}(已付){/if}.订单名细：{foreach from=$orderdet item=items}{$items[\'goodsname\']}:{$items[\'goodscount\']}份,{/foreach}.联系电话：{$orderinfo[\'buyerphone\']}.联系地址：{$orderinfo[\'buyeraddress\']}.备注:{$orderinfo[\'content\']}',
  'userbuytpl' => '【筷叫餐】{$orderinfo[\'buyername\']}在{$sitename}于{$orderinfo[\'addtime\']|date_format:"%Y-%m-%d %H:%M:%S"}下单成功,订单总价：{$orderinfo[\'allcost\']}元,订单编号{$orderinfo[\'dno\']}，配送时间：{$orderinfo[\'posttime\']|date_format:"%Y-%m-%d %H:%M:%S"}。{if $orderinfo[\'paytype_name\'] == \'open_acout\' }余额消费{$orderinfo[\'allcost\']}元,剩余余额{$memberinfo[\'cost\']-$orderinfo[\'allcost\']}元.{/if}',
...
<tr><td><strong>操作：</strong></td><td><a href="{$surelink}"><span style="3px 5px 3px 5px;backgroud:#f60;color:#fff;">确认制作</span></a>      <a href="{$closelink}"><span style="">取消制作</span></a></td></tr>
  ...
  'phoneunorder' => '【筷叫餐】{$dno}订单取消,原因{$reason},取消时间:{time()|date_format:"%Y-%m-%d %H:%M:%S"}',
  'usercodetpl' => '验证码{$code},【丰跑服务】',
  ...
  'noticesend' => '您的外卖正火速赶来。在此【筷叫餐】祝您生活愉快！',
  'noticeunmake' => '您在【筷叫餐】的订单{$dno}，由于“{$shopname}”订单爆满，来不及送货。请稍后再试，给您带来不便敬请谅解。',
  'noticeunback' => '您在【筷叫餐】的订单{$dno}，由于已制作，无法退款，给您带来不便敬请谅解！',
  'noticeback' => '',
)?>
