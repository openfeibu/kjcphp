<?php return array (
  ...
  'userbuytpl' => '【外卖人】{$orderinfo[\'buyername\']}在{$sitename}于{$orderinfo[\'addtime\']|date_format:"%Y-%m-%d %H:%M:%S"}下单成功,订单总价：{$orderinfo[\'allcost\']}元,订单编号{$orderinfo[\'dno\']}，配送时间：{$orderinfo[\'posttime\']|date_format:"%Y-%m-%d %H:%M:%S"}。{if $orderinfo[\'paytype_name\'] == \'open_acout\' }余额消费{$orderinfo[\'allcost\']}元,剩余余额{$memberinfo[\'cost\']-$orderinfo[\'allcost\']}元.{/if}',
...
<tr><td><strong>操作：</strong></td><td><a href="{$surelink}"><span style="3px 5px 3px 5px;backgroud:#f60;color:#fff;">确认制作</span></a>      <a href="{$closelink}"><span style="">取消制作</span></a></td></tr>
  ...
  'phoneunorder' => '【外卖人】{$dno}订单取消,原因{$reason},取消时间:{time()|date_format:"%Y-%m-%d %H:%M:%S"}',
  ...
  'noticeunmake' => '您在【外卖人】的订单{$dno}，由于“{$shopname}”订单爆满，来不及送货。请稍后再试，给您带来不便敬请谅解。',
  'noticeunback' => '您在【外卖人】的订单{$dno}，由于已制作，无法退款，给您带来不便敬请谅解！',
  'noticeback' => '',
)?>