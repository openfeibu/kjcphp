<?php return array (
  ...
  'shopphonetpl' => '{$orderinfo[\'buyername\']}订单总价：{$orderinfo[\'allcost\']}元{if $orderinfo[\'paystatus\'] == 0}(未付){else}(已付){/if}.订单名细：{foreach from=$orderdet item=items}{$items[\'goodsname\']}:{$items[\'goodscount\']}份,{/foreach}.联系电话：{$orderinfo[\'buyerphone\']}.联系地址：{$orderinfo[\'buyeraddress\']}.备注:{$orderinfo[\'content\']}',
