...
function time_select(start, start_quarter, end, end_quarter) {
    ...
    if (start_quarter > 0) {
        for (var q = start_quarter; q <= 3; q++) {
            if (q == 3) {
                html += '<option value='+start + ':' + q * 15 + '-' + (start + 1) + ':00'+'>' + start + ':' + q * 15 + '-' + (start + 1) + ':00' + '</option>';
            } else {
                html += '<option value='+start + ':' + q * 15 + '-' + start + ':' + (q + 1) * 15+'>' + start + ':' + q * 15 + '-' + start + ':' + (q + 1) * 15 + '</option>';
            }
        }
        if(start+1<end){
            for (var i = start + 1; i < end; i++) {
                for (var q = 0; q <= 3; q++) {
                    ...
                    html += '<option value='+begin_time + '-' + end_time+'>' + begin_time + '-' + end_time + '</option>';
                }
            }
        }
        ...
        for (var q = 0; q < end_quarter; q++) {
            if (q == 0) {
                html += '<option value='+end + ':00-' + end + ':' + (q + 1) * 15+'>' + end + ':00-' + end + ':' + (q + 1) * 15 + '</option>';
            } else {
                html += '<option value='+end + ':' + q * 15 + '-' + end + ':' + (q + 1) * 15+'>' + end + ':' + q * 15 + '-' + end + ':' + (q + 1) * 15 + '</option>';
            }
        }
    }else{
        for (var i = start; i < end; i++) {
...
window.ele = {
    init: function () {
        window.cart_goods =  JSON.parse(sessionStorage["ele"]) || {};
    },
    save : function(){
        sessionStorage['ele'] = JSON.stringify(cart_goods);
        var cookie_cart = {};
