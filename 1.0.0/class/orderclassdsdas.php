  ...
  //关闭订单
	...
	function noticemake($orderid){
		...
		if($orderinfo['pstype'] == 0 && $orderinfo['is_goshop'] == 0){//网站配送自动生成配送单
		    $checkps =  $this->ordmysql->select_one("select *  from ".Mysite::$app->config['tablepre']."orderps  where orderid= '".$orderid."'   ");
			if(empty($checkps) && $orderinfo['is_dingtai'] != 1){
     ...
     $data['areaids'] = $info['areaids'];
	   $data['admin_id'] = $info['shopinfo']['admin_id']; 
		 ...
		 $data['shoplat'] = $info['shopinfo']['lat']; 
               /*   $data['is_ycps'] = $info['ycps'];
                 
                 $data['ycpsmoney'] = $info['ycpsmoney'];
		         $data['dinersnum'] = $info['dinersnum'];
                 $data['seattype'] = $info['seattype'];
                
                 $data['goshopdate'] = $info['goshopdate'];
                 $data['goshoptime'] = $info['goshoptime']; */
...
//		    $sellrule->setdata($info['shopinfo']['id'],$data['shopcost'],$info['shopinfo']['shoptype']);
	  ...
	  $fupscost = isset($info['addpscost'])?$info['addpscost']:0;
          if($data['is_ycps'] == 1){
        $data['allcost'] = $allcost + $data['shopps'] + $fupscost + $data['bagcost'] + Mysite::$app->config['ycpsmoney'];    
        }else{
			  ...
			  if($data['is_make'] == 1){
				  ...
				  if($auto_send == 1){
					 $this->writewuliustatus($orderid,6,$data['paytype']);//订单审核后自动 商家接单后自动发货
					  ...
					  //自动生成配送单------|||||||||||||||-----------------------
					  if($data['pstype'] == 0 && $data['is_dingtai'] !=1){//网站配送自动生成配送费
		...
		$cmd['product_id'] = isset($value['gg'])?$value['gg']['id']:0;
	  	$this->ordmysql->insert(Mysite::$app->config['tablepre'].'orderdet',$cmd);
			if(isset($value['gg'])){
				...
				$jspricegg = $this->ordmysql->select_one("select jsPriceNum from ".Mysite::$app->config['tablepre']."product where id='".$cmd['product_id']."'  ");
				$jsPriceNum += $jspricegg['jsPriceNum'];
				$this->ordmysql->update(Mysite::$app->config['tablepre'].'goods','`sellcount`=`sellcount`+'.$cmd['goodscount'].' ',"id='".$cmd['goodsid']."'");

			}else{
				$jsprice = $this->ordmysql->select_one("select jsPrice from ".Mysite::$app->config['tablepre']."goods where id='".$cmd['goodsid']."'  ");
				$jsPriceNum += $jsprice['jsPrice'];
	  ...
	  $jslist['jsPriceNum'] = $jsPriceNum;
	  $this->ordmysql->update(Mysite::$app->config['tablepre'].'order',$jslist,"id='".$cmd['order_id']."'");
	  ...
	  if($userid > 0){ 
	     ...
	     if(empty($checkinfo)){
		  ...
		  if(count($newarray)>0 && $data['is_dingtai'] != 1){
			$psCls = new apppsyclass(); 
			$psCls->sendmsg(join(',',$newarray),'','订单提醒','有新订单可以处理',1);
 		  }
