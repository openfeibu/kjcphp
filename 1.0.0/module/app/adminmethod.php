...
//					}
  ...
  function sendshopmsg(){
      ...
      $appCls = new appclass(); 
	  $backinfo = $appCls->SetUid($uid)->sendNewmsg('管理员通知',$content);   
	  if($backinfo == 'ok'){
		 $this->success('success');
		 }else{
  ...
  function saveappset(){
        ...
        function appindexshow(){
               $savedata['APPindex'] = IFilter::act(IReq::get('APPindex')); 
               		$config = new config('appset.php',hopedir);
                       # print_R($savedata['APPindex']);exit;
	    $config->write($savedata);
	    $this->success('success'); 
	...
	function ajaxcheckorder(){
	  ...
	  $where = '  where ord.addtime > '.strtotime($nowday).' and ord.addtime < '.strtotime($nowday.' 23:59:59');
		...
		$shuliang1  = $this->mysql->counts("select ord.* from ".Mysite::$app->config['tablepre']."order as ord   ".$where." and ord.is_make = 0 and (ord.is_reback =0 or ord.is_reback =3)");
