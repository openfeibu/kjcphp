            ...
            foreach ($moretypelist as $key=>$value) {
                if ($value['param'] == 'weblink') {
                    ...
                    if ($value['activity'] == 'waimai') {
                        ...
                        $ajaxflag = true;
                    } elseif ($value['activity'] == 'lifehelp') {
                        $catelink = IUrl::creatUrl('/wxsite/lifeasslist');
                        $ajaxflag = false;
                    } elseif ($value['activity'] == 'shophui') {
                        $catelink = IUrl::creatUrl('/wxsite/shophui');
                        $ajaxflag = false;
                    } elseif ($value['activity'] == 'paotui') {
                        $catelink = IUrl::creatUrl('/wxsite/paotui');
                        $ajaxflag = false;
                    } elseif ($value['activity'] == 'marketlist') {
...
//             print_r($fyshoplist); exit;
    ...
    public function marketlistdata()
    {
        //2微信端,3web端
        $source = 3;
        if (strpos($_SERVER["HTTP_USER_AGENT"], 'MicroMessenger')) {
    ...
    /* 8.2新增函数 */
    public function showshoprealimg()
    {
    ...
    /* 8.3新增 */
    public function rechargepayonline()
    {
        //余额充值用手机支付宝在线支付
        $this->checkmemberlogin();
    ...
    public function ajaxroutemapshow()
    {
        $orderid = intval(IReq::get('id'));
        $order = $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."order where buyeruid='".$this->member['uid']."' and id = ".$orderid."");
        $data['psbpsyinfo'] = array();
    ...
    public function fabupaotui()
    {
        ...
        if ($pttype==1) {
            if (empty($getphone)) {
                $this->message("请填写取货电话");
            }
            if (!IValidate::suremobi($getphone)) {
    ...
    public function paotuiorder()
    {
        ...
        //
        //  echo "limit ".$pageinfo->startnum().", ".$pageinfo->getsize()."";
        $datalist = $this->mysql->getarr("select id,shopname,pttype,is_goshop,allcost,content,addtime,status,is_ping,shoptype,is_reback,buyeruid from ".Mysite::$app->config['tablepre']."order where buyeruid='".$this->member['uid']."' and shoptype = 100 order by id desc limit ".$pageinfo->startnum().", ".$pageinfo->getsize()."");
...
<body style="height:100%;width:100%;margin:0px;">
   ...
   <div style="max-width:400px;margin:0px;margin:0px auto;min-height:300px;"> '.$tempcontent.'    </div>
    ...
    public function togethersaydata()
    {
        #$this->checkwxweb();
        $pageinfo = new page();
        $pageinfo->setpage(intval(IReq::get('page')));
...
//
    ...
    public function checkpaotuicost()
    {
...
//				$list = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1 ".$ztywhere."  ".$where."   order by sort asc limit ".$pageinfo->startnum().", ".$pageinfo->getsize()." ");
    ...
    {  // 获取对应促销类型的商家
        ...
        //  limit ".$pageinfo->startnum().", ".$pageinfo->getsize()."
        $d = (date("w") ==0) ?7:date("w") ;
        $cxlist = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."rule where  status = 1 and controltype = ".$type." and ( limittype = 1 or ( limittype = 2 and ".$d."  in ( limittime )  )  or ( limittype = 3 and endtime > ".time()." and starttime < ".time().")) ");
        $shopids = array();
        ...
        if (!empty($shopids)) {
            $list = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1 and  id in (".$shopids.")  order by sort asc limit {$page},10  ");
        } else {
            $list = array();
        }
        return $list;
    ...
    public function sjapplyrz()
    {
        ...
        if (!empty($shopphone)&&!(IValidate::phone($shopphone))) {
            $this->message('errphone');
        }
        ...
        if (!empty($checkshopname)) {
            $this->message("店铺名字已存在");
        }
        if (empty($shopaddress)) {
            $this->message("请填写店铺地址");
        }
        if (empty($shoplicense)) {
    ...
    public function shangjiaapply()
    {
        ...
        if (empty($shopphone)) {
            $this->message("请填写联系电话", $link);
        }
        if (!empty($shopphone)&&!(IValidate::phone($shopphone))) {
            $this->message('errphone', $link);
        }
        $checkphone = $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."member  where phone = ".$shopphone." ");
        if (!empty($checkphone)) {
            $this->message("手机号已存在", $link);
        }
        if (empty($shopname)) {
            $this->message("请填写店铺名称", $link);
        }
        $checkshopname = $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."shop  where shopname = '".$shopname."' ");
        if (!empty($checkshopname)) {
            $this->message("店铺名字已存在", $link);
        }
        if (empty($shopaddress)) {
            $this->message("请填写店铺地址", $link);
        }
    ...
    public function ajaxnoticelist()
    {
        ...
        $list = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."information where type=1  and (   cityid='".$this->CITY_ID."'  or  cityid = 0 ) order by addtime desc limit ".$pageinfo->startnum().", ".$pageinfo->getsize()."");
        $backdata = array();
        $noticeshuliang = $this->mysql->counts("select * from ".Mysite::$app->config['tablepre']."information where type=1  and (   cityid='".$this->CITY_ID."'  or  cityid = 0 ) ");
        $backdata = array();
    ...
    /* 新增生活助手 */
    public function ajaxlifeasslist()
    {
        $pageinfo = new page();
        $pageinfo->setpage(intval(IReq::get('page')), 10);
    ...
    public function saveshopreport()
    {
        $typeidContent = IFilter::act(IReq::get('typeidContent'));
        $shopname = IFilter::act(IReq::get('shopname'));
        $content = IFilter::act(IReq::get('content'));
        $phone= IReq::get('phone');
        if ($typeidContent == null || $typeidContent == '') {
            $this->message('请选择一种投诉类型');
        }
        if (empty($content)) {
            $this->message('详细情况不能为空');
        }
        if (empty($phone)) {
    ...
    public function receivejuan()
    {    //下单成功分享优惠券 根据手机号  领取优惠券

        $orderid = intval(IReq::get('orderid'));
        $phone = IFilter::act(IReq::get('phone'));
        $orderinfo = $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."order where id=".$orderid."  ");
        if (empty($orderinfo)) {
            $this->message('获取失败，请尝试刷新页面~');
        }
        if (empty($phone)) {
            $this->message('请填写领取使用的手机号~');
        }
        if (!(IValidate::suremobi($phone))) {
    ...
    public function memsharelqjuan()
    {    //会员推广分享优惠券 根据手机号  领取优惠券
...
//var_dump($cxinfo);exit;
    ...
    //删除优惠订单id
    public function delshophuiorder()
    {
    ...
    // 8.4新增 �    �注微信领取优惠券功能  2016-07-15
    public function gzwx()
    {
    ...
    public function receivgzwxjuan()
    {
    ...
    public function discountlistdata()
    {
        ...
        $nowdate = date('Y-m-d', time());
        $checktime = $nowtime-strtotime($nowdate);
        $sql = "SELECT * FROM ".Mysite::$app->config['tablepre']."goods as g left join ".Mysite::$app->config['tablepre']."goodscx as gc on g.id = gc.goodsid join on ".Mysite::$app->config['tablepre']."shop as s on g.shopid = s.id WHERE s.stationid = ".$this->stationid." g.is_cx = 1 AND ".$nowtime." > gc.cxstarttime AND ".$nowtime." < gc.ecxendttime AND (($checktime > gc.cxstime1 AND $checktime < gc.cxetime1) OR ($checktime > gc.cxstime2 AND $checktime < gc.cxetime2)) limit ".$pageinfo->startnum().", ".$pageinfo->getsize()."  ";
        $goods_list = $this->mysql->getarr($sql);
        foreach ($goods_list as $key => $goods) {
            $goods_list[$key]['cost'] = $goods['cost']*$goods['cxzhe']*0.01;
            $goods_list[$key]['zhekou'] = $goods['cxzhe']*0.1;
