...
class method extends baseclass
{
    ...
    public function editgoodstype()
    {
        ...
        }//$this->json(array('error'=>true,'message'=>''));
        if ($what == 'name') {
    ...
    public function addgoods()
    {
        ...
        // if($data['cost'] <= 0) $this->message('请输入金额');
    ...
    public function savecxrule()
    {
        ...
        if (!empty($checkcx) && empty($cxid)) {
            $this->message('您已添加过该类型的促销活动，不可重复添加！');
        }
    ...
    public function saveshophui()
    {
        ...
        if (!in_array($controltype, array('2','3','4'))) {
            $this->message('闪惠类型错误！');
        }
    ...
    public function alltoshopgoods()
    {
        ...
        if (!empty($name)) {
            ...
            $shuliang  = $this->mysql->counts("select * from ".Mysite::$app->config['tablepre']."goodslibrary  where name like '%".$name."%'   ");
        } else {
    ...
    public function adgoodstoshop()
    {
        ...
        if ($yangshiid == 1) {
            ...
            $data['instro']   =  strip_tags($tempinfo['instro']);
    ...
    public function backcomment()   //  商家回复评论
    {
        $id = intval(IReq::get('askbackid'));
        if (empty($id)) {
            $this->message('获取失败');
    ...
    //商家实景批量上传图片
    public function shoprealimgupload()
    {
        $parent_id = intval(IReq::get('parent_id'));
    ...
    //删除商家实景图片
    public function delshoprealimg()
    {
    ...
    public function saveshopreal()
    {
        ...
        if (!(IValidate::len($data['name'], 1, 5))) {
            $this->message('分类名称最多不能超过五个字');
        }
        if ($type == 'insert') {
...
//        $data['imglist'] = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."goodsimg where goodsid=".$goodsid);
    ...
    //删除商品图片
    public function delonegoodsimg()
    {
        $imglujing = trim(IReq::get('imglujing'));
