    ...
    public function ordercontrol()
    {
        ...
        switch ($type) {
          ...
          case 'send':
              ...
              if ($orderinfo['is_make'] == 0 && $orderinfo['pttype'] == 0) {
...
//					}else{
    ...
    public function ajaxcheckorder()
    {
        ...
        $shuliang1  = $this->mysql->counts("select ord.* from ".Mysite::$app->config['tablepre']."order as ord   ".$where." and ord.is_make = 0 ");
    ...
    public function drawbacklog()
    {
        ...
        $list = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."drawbacklog   ".$where." and status !=3 order by  id desc  limit ".$pageshow->startnum().", ".$pageshow->getsize()." ");
        $data['list'] = array();
        foreach ($list as $key=>$value) {
    ...
    ***************/
    ...
    //取消店铺的提现申请
    public function saveshoptxreason()
    {
