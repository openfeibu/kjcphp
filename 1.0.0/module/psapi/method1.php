...
class method   extends baseclass
{
    ...
    function ordercontrol()
    {
        ...
        $clerk_phone = intval(IFilter::act(IReq::get('clerk_phone')));//配送员手机
	...
	logwrite($info); 
        if($this->checkpsb() && $this->checkorder()) { 
            ...
            if ($dotype == 'qiangorder') {//配送员抢单
                ...
                $statusdata['ststusdesc'] = $clerk_name . '抢单成功,联系电话' . $clerk_phone;
                ...
                $wxbuyer = $this->mysql->select_one("select *  from " . Mysite::$app->config['tablepre'] . "wxuser  where uid= '" . $ordercheck['buyeruid'] . "'   ");
                $wx_s = new wx_s();
                $orderinfo1 = $this->mysql->select_one("select psusername,psemail  from " . Mysite::$app->config['tablepre'] . "order where id = '" . $orderid . "'   ");
                if (!empty($orderinfo1)) {
                    $content = '配送员【' . $orderinfo1['psusername'] . '】已抢单，联系电话：' . $orderinfo1['psemail'] . '，在此【外卖人】祝您生活愉快！';
                } else {
                    $content = '配送员已抢单，在此【外卖人】祝您生活愉快！';
                }

              $wx_s->sendmsg($content, $wxbuyer['openid']);
                ...
                $statusdata['orderid'] = $orderid;
                $statusdata['statustitle'] = "配送员已接指派订单";
                ...
                if ($ordercheck['buyeruid'] > 0) {
                   ...
                   $orderinfo1 = $this->mysql->select_one("select psusername,psemail  from " . Mysite::$app->config['tablepre'] . "order where id = '" . $orderid . "'   ");
                    if(!empty($orderinfo1)) {
                         $content =
                         '配送员【'.$orderinfo1['psusername'].'】已取货，联系电话：'.$orderinfo1['psemail'].'，在此【外卖人】祝您生活愉快！';
                     }else{
                         $content =
                         '配送员已取货，在此【外卖人】祝您生活愉快！';
                     }
...
//                $orderdata['status'] = 3;
                ...
                $wxbuyer = $this->mysql->select_one("select *  from ".Mysite::$app->config['tablepre']."wxuser  where uid= '".$ordercheck['buyeruid']."'   ");
                  ...
                  $orderinfo1 = $this->mysql->select_one("select psusername,psemail  from " . Mysite::$app->config['tablepre'] . "order where id = '" . $orderid . "'   ");
                    if(!empty($orderinfo1)) {
                         $content =
                         '配送员【'.$orderinfo1['psusername'].'】已送达，联系电话：'.$orderinfo1['psemail'].'，在此【'.Mysite::$app->config['sitename'].'】祝您生活愉快！';
                     }else{
                         $content =
                         '配送员已取货，在此【'.Mysite::$app->config['sitename'].'】祝您生活愉快！';
                     }
