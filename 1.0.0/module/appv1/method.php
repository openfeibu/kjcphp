	...
	*/
	...
	function fastlogin(){
		...
		if($phonecls->checkcode($code)){
			...
			$tjyhj = $this->mysql->counts("select * from ".Mysite::$app->config['tablepre']."juan where uid='".$member['uid']."' ");
...
//		$goodslist =  $this->mysql->getarr("select id,typeid,name,count,cost,bagcost,img,have_det from ".Mysite::$app->config['tablepre']."goods where shopid='".$shopinfo['id']."' and typeid = ".$typeid." order by id desc  limit   ".$this->pageCls->startnum().", ".$this->pageCls->getsize()."  ");
	...
	*/
	...
	function appuserinfo(){
		...
		unset($backinfo['password']);
		$tjyhj = $this->mysql->counts("select * from ".Mysite::$app->config['tablepre']."juan where uid='".$backinfo['uid']."' ");
...
function getshopnew(){
				...
				if($shopinfo['shoptype'] == 0){ 
					...
					if($ios == 'marketos'){
						...
						if($value['parent_id'] == 0){
							...
							$goodsdet = $this->mysql->getarr("select id,typeid,name,count,cost,img,point,bagcost,is_cx,sellcount,shopid,have_det,product_attr,descgoods,goodattr from ".Mysite::$app->config['tablepre']."goods where typeid = ".$value['id']."  and shopid =".$shopinfo['id']."   order by id asc ");
							$temparr = array();
							foreach($goodsdet as $k=>$v){
                                if(empty($v['goodattr'])){
                                    $shopdefaultattr = $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."shop where id = ".$shopid." ");
                                    if(!empty($goodsinfo)){
                                       $v['goodsattr'] =  $shopdefaultattr['goodattrdefault'];
                                    }else{
                                        $v['goodsattr'] = '份';
                                    }
                                }
	...
	function newshop(){
		...
		*/
		...
		if($goodstype > 0){
		    ...
		    $attrinfo = $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."shoptype where   id=".$goodstype." order by orderid desc limit 0,1000"); 
		    if(!empty($attrinfo)){
			   $shoptype = $attrinfo['cattype'];
			}
		...
		$where = Mysite::$app->config['plateshopid'] > 0? $where.' and shopid != '.Mysite::$app->config['plateshopid'] .' ':$where;
		$tempdd = array();
		if($shopopentype == 0){
			$tempdd[] = $this->mysql->getarr("select a.shopid,b.id,b.shopname,b.sellcount,point,pointcount,b.virtualsellcounts,b.is_open,b.starttime,b.pointcount as pointsellcount,lat,lng,a.is_orderbefore,a.limitcost,b.shoplogo,a.is_hot,a.is_com,a.is_new,a.maketime,a.pradius,b.lat,b.lng,a.sendtype,a.pscost,a.pradiusvalue,a.arrivetime,b.address,b.point,b.shoptype  from ".Mysite::$app->config['tablepre']."shopfast as a left join ".Mysite::$app->config['tablepre']."shop as b  on a.shopid = b.id and b.is_recom = 1 ".$where."   ".$orderarray[$ordertype]."   limit ".$this->pageCls->startnum().", ".$this->pageCls->getsize()."");
			$tempdd[] = $this->mysql->getarr("select a.shopid,b.id,b.shopname,b.sellcount,point,pointcount,b.virtualsellcounts,b.is_open,b.starttime,b.pointcount as pointsellcount,lat,lng,a.is_orderbefore,a.limitcost,b.shoplogo,a.is_hot,a.is_com,a.is_new,a.maketime,a.pradius,b.lat,b.lng,a.sendtype,a.pscost,a.pradiusvalue,a.arrivetime,b.address,b.point,b.shoptype  from ".Mysite::$app->config['tablepre']."shopfast as a left join ".Mysite::$app->config['tablepre']."shop as b  on a.shopid = b.id  and b.is_recom != 1 ".$where."   ".$orderarray[$ordertype]."   limit ".$this->pageCls->startnum().", ".$this->pageCls->getsize()."");
		 
		}elseif($shopopentype == 1){
			$tempdd[] = $this->mysql->getarr("select a.shopid,b.id,b.shopname,b.sellcount,point,pointcount,b.virtualsellcounts,b.is_open,b.starttime,b.pointcount as pointsellcount,lat,lng,a.is_orderbefore,a.limitcost,b.shoplogo,a.is_hot,a.is_com,a.is_new,a.maketime,a.pradius,b.lat,b.lng,a.sendtype,a.pscost,a.pradiusvalue,a.arrivetime,b.address,b.point,b.shoptype  from ".Mysite::$app->config['tablepre']."shopmarket as a left join ".Mysite::$app->config['tablepre']."shop as b  on a.shopid = b.id and b.is_recom = 1 ".$where."   ".$orderarray[$ordertype]."   limit ".$this->pageCls->startnum().", ".$this->pageCls->getsize()."");
			$tempdd[] = $this->mysql->getarr("select a.shopid,b.id,b.shopname,b.sellcount,point,pointcount,b.virtualsellcounts,b.is_open,b.starttime,b.pointcount as pointsellcount,lat,lng,a.is_orderbefore,a.limitcost,b.shoplogo,a.is_hot,a.is_com,a.is_new,a.maketime,a.pradius,b.lat,b.lng,a.sendtype,a.pscost,a.pradiusvalue,a.arrivetime,b.address,b.point,b.shoptype  from ".Mysite::$app->config['tablepre']."shopmarket as a left join ".Mysite::$app->config['tablepre']."shop as b  on a.shopid = b.id and b.is_recom != 1 ".$where."   ".$orderarray[$ordertype]."   limit ".$this->pageCls->startnum().", ".$this->pageCls->getsize()."");
		}else{
			$tempwherexxx =  Mysite::$app->config['plateshopid'] > 0? ' and id != '.Mysite::$app->config['plateshopid'] .' ':'';
			$list = array();
			...
			if(strpos($userAgent,"iPhone") || strpos($userAgent,"iPad") || strpos($userAgent,"iPod")){
				...
				$ios_waiting =   Mysite::$app->config['ios_waiting'];
				if($source == 1 && $ios_waiting == true){
				    $tempc = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1 ".$tempwherexxx." and endtime > ".time()." and is_recom=1   order by sort asc");
				    $tempb = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1 ".$tempwherexxx."  and endtime > ".time()." and is_recom!=1   order by sort asc"); 
				}else{
			        $tempc = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1 ".$tempwherexxx." and endtime > ".time()." and is_recom=1    order by (`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." ) asc");#and  SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradiusa`*0.01094)
					$tempb = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1 ".$tempwherexxx." and endtime > ".time()." and is_recom!=1    order by (`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." ) asc");# and  SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradiusa`*0.01094)
				} 
		...
		#	
			if(is_array($tempc)){
				foreach($tempc as $key=>$value){
					...
					if(!empty($tempshop) && count($tempshop) > 0){
	...
	//			echo '<br>';
				...
				$value['pstime'] = date('H:i',$nowhout+$timelist[0]['s']);

				
				$ordercounts = $this->mysql->counts("select id from ".Mysite::$app->config['tablepre']."order where shopid = ".$value['id']." and status = 3 ");
				$value['sellcount'] = $value['virtualsellcounts']+$ordercounts;
	...
	function myjuan(){
		...
		$cximglist = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."juan where uid =".$backinfo['uid']." order by id desc   limit ".$this->pageCls->startnum().", ".$this->pageCls->getsize()."");
		$backdata = array();
	...
	function getcx(){
	  ...
	  //获取所有促销 
		...
		$yhjlist = $this->mysql->getarr("select id,name,cost,limitcost,endtime,paytype  from ".Mysite::$app->config['tablepre']."juan  where uid ='".$backinfo['uid']."'  and status = 1 and endtime > ".time()." and limitcost < ".$backdata['surecost']."  order by id desc limit 0,20");
	...
	function sendregphone(){
		...
		if($type == 8){
			$phone = trim(IFilter::act(IReq::get('tphone')));  
		}
...
if($showtype == 1){    // 加载商品
	...
	function getNoticeList(){  // 获取通知列表 
		...
		$pagesize = empty($pagesize)?10:$pagesize;
		$this->pageCls->setpage(intval(IReq::get('page')),$pagesize);
	    $noticeList = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."information where type = 1  ".$citywhere."  order by orderid asc  limit ".$this->pageCls->startnum().", ".$this->pageCls->getsize()." ");
...
//8.3新增到店消费制作订单app接口  lzh  2016-6-7
	...
	function ordersharejuan(){
		...
		$orderid = intval(IFilter::act(IReq::get('orderid')));
		$userordersharejuan = Mysite::$app->config['userordersharejuan'];
		...
		if( !empty($checkinfosendjuan) && $userordersharejuan == 0 &&  !empty($shareinfo)  ){
...
//新增函数 
	...
	function newwaimairenindex(){
		$adcode = trim(IFilter::act(IReq::get('adcode')));
		$lat = IFilter::act(IReq::get('lat'));
		$lng = IFilter::act(IReq::get('lng'));
		$lat = empty($lat)?0:$lat;
		$lng =empty($lng)?0:$lng;
		$data['ios_waiting'] = false;
		$userAgent = $_SERVER['HTTP_USER_AGENT'];
		if(strpos($userAgent,"iPhone") || strpos($userAgent,"iPad") || strpos($userAgent,"iPod")){
			$ios_waiting =   Mysite::$app->config['ios_waiting'];
			if($ios_waiting == true){
				$data['ios_waiting'] = true;
				$adcode = Mysite::$app->config['default_cityid'];
				$lat = Mysite::$app->config['maplat'];
				$lng = Mysite::$app->config['maplng'];
				$data['ios_lat'] = $lat;
				$data['ios_lng'] = $lng;
				$data['ios_adcode'] =$adcode;
				$data['ios_cityname'] = Mysite::$app->config['cityname'];
			}

		}
		$lat = is_float($lat)?$lat:0;
		$lng = is_float($lng)?$lng:0;
		$cityid = 0;
		$cityinfo = array();
		if( !empty($adcode) ){
			$areacodeone =  $this->mysql->select_one("select * from ".Mysite::$app->config['tablepre']."areacode where id=".$adcode."  ");
		...
		//typy=1的是幻灯片
		//typy=1的是幻灯片
		$data['appadv'] = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."appadv where id > 0 ".$citywhere." order by orderid asc");
		...
		// 专题页
// 	   $data['ztylist'] =   $this->mysql->getarr("select id,name,indeximg as img,showtype,cx_type,is_custom,cityid from ".Mysite::$app->config['tablepre']."specialpage where is_show=1 ".$citywhere." and ((is_custom =0 ) or (  is_custom =1 and showtype =1) or (cx_type = 9) )  order by orderid  asc");
		$data['ztylist'] = $this->mysql->getarr("select id,name,indeximg as img,showtype,cx_type,is_custom,cityid  from ".Mysite::$app->config['tablepre']."specialpage where is_show=1  and (   cityid='".$cityid."'  or  cityid = 0 )   order by orderid  asc");
       ...
       */
		...
		if($shopopentype == 0){
			...
			$tempwherexxx =  Mysite::$app->config['plateshopid'] > 0? ' and id != '.Mysite::$app->config['plateshopid'] .' ':'';
			$tempwherexxx .=  " and admin_id = '".$cityid."'  ";
			$list = array();
			...
			if(strpos($userAgent,"iPhone") || strpos($userAgent,"iPad") || strpos($userAgent,"iPod")){
				...
				if($source == 1 && $ios_waiting == true){
					...
					$tempc = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1 ".$tempwherexxx." and endtime > ".time()." and is_recom=1   and  SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradiusa`*0.01094) order by (`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." ) asc");
					$tempb = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1 ".$tempwherexxx." and endtime > ".time()." and is_recom!=1   and  SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradiusa`*0.01094)  order by (`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." ) asc");
				}
			}else if(strpos($userAgent,"Android")){
				$tempc = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1 ".$tempwherexxx." and endtime > ".time()."  and is_recom=1 and  SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradiusa`*0.01094) order by (`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." ) asc");
				$tempb = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1 ".$tempwherexxx." and endtime > ".time()."  and is_recom!=1 and  SQRT((`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." )) < (`pradiusa`*0.01094) order by (`lat` -".$lat.") * (`lat` -".$lat." ) + (`lng` -".$lng." ) * (`lng` -".$lng." ) asc");
		...
		foreach($tempdd as $kvc=>$list){
			foreach($list as $key=>$value){
				...
				if( $value['sendtype'] == 1 ){
				     ...
				     $valuelist = empty($this->platpsinfo['radiusvalue'])? '':unserialize($this->platpsinfo['radiusvalue']);	 
				}
