...
//orderid
   ...
   function outpstjorder()
	...
	function ordertodaytable(){
		...
		$shuliang  = $this->mysql->counts("select psd.*,ord.*,mb.username as acountname from ".Mysite::$app->config['tablepre']."orderps as psd left join ".Mysite::$app->config['tablepre']."order as ord on psd.orderid = ord.id  left join  ".Mysite::$app->config['tablepre']."member as mb on mb.uid = ord.buyeruid   ".$where." ");
