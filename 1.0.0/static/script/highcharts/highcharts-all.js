...
var HighchartsAdapter = function() {
		function p(c) {
			...
			function a(a, c) {
				var f = a.HCEvents,
					i, g, k, j;
				if (a.removeEventListener) i = b;
				else if (a.attachEvent) i = d;
				else return;
				c ? (g = {}, g[c] = !0) : g = f;
			...
			init: function(c) {
				...
				o.prototype = {
					update: function() {
						...
						var d = this.elem,
							a = d.element;
						if (n[this.prop]) n[this.prop](this);
						else b && a ? d.attr("d", c.step(b[0], b[1], this.now, this.toD)) : d.attr ? a && d.attr(this.prop, this.now) : (b = {}, b[this.prop] = this.now + this.unit, Highcharts.css(d, b));
					...
					step: function(b) {
						...
						var e = this.elem,
							h;
						if (e.stopAnimation || e.attr && !e.element) a = !1;
						else if (b || c >= a.duration + this.startTime) {
							this.now = this.end;
...
(function() {
	...
	function F(a, b, c) {
		var d, e;
		if (Ga(b)) s(c) ? a.setAttribute(b, c) : a && a.getAttribute && (e = a.getAttribute(b));
	...
	function Ba(a, b, c, d) {
		var e = K.numberFormat,
			f = E.lang,
			g = +a || 0,
			h = b === -1 ? (g.toString().split(".")[1] || "").length : isNaN(b = Q(b)) ? 2 : b,
			i = c === void 0 ? f.decimalPoint : c,
			f = d === void 0 ? f.thousandsSep : d,
			j = g < 0 ? "-" : "",
			k = String(y(g = Q(g).toFixed(h))),
	...
	var u, x = document,
		...
		Hb = G.opera,
		Aa = /msie/i.test(wa) && !Hb,
		hb = x.documentMode === 8,
		tb = /AppleWebKit/.test(wa),
		Ua = /Firefox/.test(wa),
		Ib = /(Mobile|Android|Windows Phone)/.test(wa),
		xa = "http://www.w3.org/2000/svg",
		ba = !! x.createElementNS && !! x.createElementNS(xa, "svg").createSVGRect,
		Ob = Ua && parseInt(wa.split("Firefox/")[1], 10) < 4,
		ga = !ba && !Aa && !! x.createElement("canvas").getContext,
		Za, $a, Jb = {},
		ub = 0,
		db, E, cb, va, vb, A, ha, sa = function() {
			return u
		},
		W = [],
		ab = 0,
		...
		Pb = /^[0-9]+$/,
		Qb = "stroke-width",
		Da, eb, Sa, pb, qb, rb, Xa, fb, gb, Cb, Db, sb, Eb, Fb, H = {},
		K;
	...
	ha = function(a, b) {
		var c = "Highcharts error #" + a + ": www./errors/" + a;
		if (b) throw c;
	...
	E = {
		...
		global: {
			useUTC: !0,
			canvasToolsURL: "http://code./4.0.4/modules/canvas-tools.js",
			VMLRadialGradientURL: "http://code./4.0.4/gfx/vml-radial-gradient.png"
		},
		chart: {
			borderColor: "#4572A7",
			borderRadius: 0,
	...
	S.prototype["stroke-widthSetter"] = S.prototype.strokeSetter = function(a, b, c) {
		this[b] = a;
		if (this.stroke && this["stroke-width"]) this.strokeWidth = this["stroke-width"], S.prototype.fillSetter.call(this, this.stroke, "stroke", c), c.setAttribute("stroke-width", this["stroke-width"]), this.hasStroke = !0;
	...
	ta.prototype = {
		...
		buildText: function(a) {
			...
			!f && !m && e.indexOf(" ") === -1 ? b.appendChild(x.createTextNode(e)) : (h = /<.*style="([^"]+)".*>/, i = /<.*href="(http[^"]+)".*>/, l && !a.added && this.box.appendChild(b), e = f ? e.replace(/<(b|strong)>/g, '<span style="font-weight:bold">').replace(/<(i|em)>/g, '<span style="font-style:italic">').replace(/<a/g, "<span").replace(/<\/(b|strong|i|em|a)>/g, "</span>").split(/<br.*?>/g) : [e], e[e.length - 1] === "" && e.pop(), q(e, function(e, f) {
				var g, n = 0,
		...
		fontMetrics: function(a, b) {
			...
			var a = /px/.test(a) ? y(a) : /em/.test(a) ? parseFloat(a) * 12 : 12,
				c = a < 24 ? a + 4 : v(a * 1.2),
				d = v(c * 0.8);
			return {
				h: c,
	...
	r(S.prototype, {
		...
		setSpanRotation: function(a, b, c) {
			var d = {},
				e = Aa ? "-ms-transform" : tb ? "-webkit-transform" : Ua ? "MozTransform" : Hb ? "-o-transform" : "";
	...
	if (!ba && !ga) {
		Z = {
			...
			getSpanCorrection: function(a, b, c, d, e) {
				...
				e && e !== "left" && (this.xCorr -= a * c * (f < 0 ? -1 : 1), d && (this.yCorr -= h * c * (g < 0 ? -1 : 1)), B(this.element, {
					textAlign: e
				}))
			},
			pathToVML: function(a) {
				for (var b = a.length, c = []; b--;) if (ja(a[b])) c[b] = v(a[b] * 10) - 5;
				else if (a[b] === "Z") c[b] = "x";
		...
		var ia = {
			...
			color: function(a, b, c, d) {
				...
				if (i) {
					...
					if (c === "fill") if (i === "gradient") c = n.x1 || n[0] || 0, a = n.y1 || n[1] || 0, m = n.x2 || n[2] || 0, n = n.y2 || n[3] || 0, C = 'angle="' + (90 - V.atan((n - a) / (m - c)) * 180 / oa) + '"', u();
					else {
						var j = n.r,
							r = j * 2,
							v = j * 2,
							...
							w, j = function() {
								...
								C = 'src="' + E.global.VMLRadialGradientURL + '" size="' + r + "," + v + '" origin="0.5,0.5" position="' + x + "," + z + '" color2="' + J + '" ';
								u()
							};
						d.added ? j() : d.onAdd = j;
						j = D
					} else j = k
				} else if (g.test(a) && b.tagName !== "IMG") f = ya(a), h = ["<", c, ' opacity="', f.get("a"), '"/>'], $(this.prepVML(h), null, null, b), j = f.get("rgb");
				else {
	...
	Ta.prototype = {
		...
		render: function(a, b, c) {
			var d = this.axis,
				...
				x = w.x,
				w = w.y,
				z = g && x === d.pos + d.len || !g && w === d.pos ? -1 : 1,
				c = p(c, 1);
			this.isActive = !0;
			if (o) {
			...
			if (i && !isNaN(x)) i.xy = w = this.getLabelPosition(x, w, i, g, k, v, a, s), this.isFirst && !this.isLast && !p(e.showFirstLabel, 1) || this.isLast && !this.isFirst && !p(e.showLastLabel, 1) ? r = !1 : !d.isRadial && !k.step && !k.rotation && !b && c !== 0 && (r = this.handleOverflow(a, w)), s && a % s && (r = !1), r && !isNaN(w.y) ? (w.opacity = c, i[this.isNew ? "attr" : "animate"](w), this.isNew = !1) : i.attr("y", -9999)
		},
		destroy: function() {
			Pa(this, this.axis)
		}
	...
	K.PlotLineOrBand.prototype = {
		render: function() {
			...
			} else if (o && o.length && (a.svgElem = m = u.path(o).attr(r).add(), C)) for (p in d = function(b) {
				m.on(b, function(c) {
					C[b].apply(a, [c])
				})
			}, C) d(p);
	...
	na.prototype = {
		...
		defaultLabelFormatter: function() {
			...
			else if (f && a >= 1E3) for (; f-- && g === u;) c = Math.pow(1E3, f + 1), a >= c && e[f] !== null && (g = Ba(b / c, -1) + e[f]);
			...
			q(a.series, function(c) {
				if (c.visible || !b.options.chart.ignoreHiddenSeries) {
					...
					if (a.isXAxis) {
						...
						if (s(d)) if (a.dataMin >= d) a.dataMin = d, a.ignoreMinPadding = !0;
		...
		setTickPositions: function(a) {
			...
			b.min === b.max || b.min === void 0 || b.max === void 0 ? b.tickInterval = 1 : j && !m && Y === b.linkedParent.options.tickPixelInterval ? b.tickInterval = b.linkedParent.tickInterval : (b.tickInterval = p(m, J ? 1 : (b.max - b.min) * Y / t(b.len, Y)), !s(m) && b.len < Y && !this.isRadial && !this.isLog && !J && e && f && (D = !0, b.tickInterval /= 4));
			...
			if (!m && b.tickInterval < o) b.tickInterval = o;
			...
			if (!a)!b.ordinalPositions && (b.max - b.min) / b.tickInterval > t(2 * b.len, 200) && ha(19, !0), a = h ? b.getTimeTicks(b.normalizeTimeTickInterval(b.tickInterval, d.units), b.min, b.max, d.startOfWeek, b.ordinalPositions, b.closestPointRange, !0) : g ? b.getLogTickPositions(b.tickInterval, b.min, b.max) : b.getLinearTickPositions(b.tickInterval, b.min, b.max), D && a.splice(1, a.length - 2), b.tickPositions = a;
		...
		getThreshold: function(a) {
			var b = this.isLog,
				c = b ? ka(this.min) : this.min,
				b = b ? ka(this.max) : this.max;
		...
		getOffset: function() {
			...
			J[h] = t(J[h], a.axisTitleMargin + l + C * a.offset, g);
		...
		render: function() {
			var a = this,
				...
				D = a.tickmarkOffset,
				J = f.lineWidth,
				C = d.hasRendered && s(a.oldMin) && !isNaN(a.oldMin),
				r = a.hasData,
				t = a.showAxis,
				v, w = f.labels.overflow,
			...
			if (J) b = a.getLinePath(J), a.axisLine ? a.axisLine.animate({
				d: b
			...
			if (k && t) k[k.isNew ? "attr" : "animate"](a.getTitlePosition()), k.isNew = !1;
	...
	r(na.prototype, {
		...
		addPlotBandOrLine: function(a, b) {
			var c = (new K.PlotLineOrBand(this, a)).render(),
				d = this.userOptions;
	...
	na.prototype.getTimeTicks = function(a, b, c, d) {
		...
		if (s(b)) {
			...
			h = i[gb]();
			for (var d = i.getTime(), l = i[fb](), n = i[Xa](), m = (A.day + (g ? Sa : i.getTimezoneOffset() * 6E4)) % A.day; d < c;) e.push(d), j === A.year ? d = eb(h + b * k, 0) : j === A.month ? d = eb(h, l + b * k) : !g && (j === A.day || j === A.week) ? d = eb(h, l, n + b * k * (j === A.day ? 1 : 7)) : d += j * k, b++;
			e.push(d);
			q(wb(e, function(a) {
	...
	Nb.prototype = {
		...
		tooltipHeaderFormatter: function(a) {
			var b = a.series,
				...
				e = c.xDateFormat,
				f = b.xAxis,
				g = f && f.options.type === "datetime" && ja(a.key),
				c = c.headerFormat,
	...
	Wa.prototype = {
		init: function(a, b) {
			...
			this.pinchDown = [];
			this.lastValidTouch = {};
			if (K.Tooltip && b.tooltip.enabled) a.tooltip = new Nb(a, b.tooltip), this.followTouchMove = b.tooltip.followTouchMove;
			this.setDOMEvents()
		},
		...
		runPointActions: function(a) {
			...
			if (d && this.options.tooltip.shared && (!h || !h.noSharedTooltip)) {
				f = [];
				i = c.length;
				for (j = 0; j < i; j++) if (c[j].visible && c[j].options.enableMouseTracking !== !1 && !c[j].noSharedTooltip && c[j].singularTooltips !== !0 && c[j].tooltipPoints.length && (e = c[j].tooltipPoints[l]) && e.series) e._dist = Q(l - e.clientX), k = L(k, e._dist), f.push(e);
		...
		setDOMEvents: function() {
			...
			}, ab === 1 && N(x, "touchend", a.onDocumentTouchEnd)
		},
		destroy: function() {
			var a;
	...
	if (G.PointerEvent || G.MSPointerEvent) {
		...
		Na(Wa.prototype, "init", function(a, b, c) {
			a.call(this, b, c);
			(this.hasZoom || this.followTouchMove) && B(b.container, {
				"-ms-touch-action": P,
				"touch-action": P
			})
		});
	...
	Ya.prototype = {
		...
		getChartSize: function() {
			...
			this.chartWidth = t(0, b || this.containerWidth || 100%);
			this.chartHeight = t(0, p(a, this.containerHeight > 19 ? this.containerHeight : 400))
		},
		cloneRenderTo: function(a) {
			var b = this.renderToClone,
				c = this.container;
			a ? b && (this.renderTo.appendChild(c), Qa(b), delete this.renderToClone) : (c && c.parentNode === this.renderTo && this.renderTo.removeChild(c), this.renderToClone = b = this.renderTo.cloneNode(0), B(b, {
				position: "absolute",
				top: "-9999px",
				display: "block"
		...
		drawChartBox: function() {
			...
			if (k) f ? f.animate(t) : this.plotBackground = b.rect(o, p, q, r, 0).attr({
				fill: k
			}).add().shadow(a.plotShadow);
			if (l) h ? h.animate(t) : this.plotBGImage = b.image(l, o, p, q, r).add();
			s ? s.animate({
				width: v.width,
				height: v.height
		...
		renderLabels: function() {
			...
			b.items && q(b.items, function(c) {
				var d = r(b.style, c.style),
					e = y(d.left) + a.plotLeft,
					f = y(d.top) + a.plotTop + 12;
				delete d.left;
				delete d.top;
		...
		isReadyToRender: function() {
			var a = this;
			return !ba && G == G.top && x.readyState !== "complete" || ga && !G.canvg ? (ga ? Mb.push(function() {
				a.firstRender()
	...
	O.prototype = {
			...
			if (d !== !1 && h && g === h && !e.cropped && !e.hasGroupedData) q(a, function(a, b) {
				...
				if (o && h > o) {
					...
					if (ja(k)) {
						...
						e.xIncrement = n
					} else if (Ha(k)) if (s) for (c = 0; c < h; c++) i = a[c], r[c] = i[0], t[c] = i.slice(1, s + 1);
					else for (c = 0; c < h; c++) i = a[c], r[c] = i[0], t[c] = i[1];
					else ha(12)
		...
		processData: function(a) {
			...
			if (l && this.sorted && (!i || d > i || this.forceCrop)) if (b[d - 1] < m || b[0] > n) b = [], c = [];
		...
		drawPoints: function() {
			var a, b = this.points,
				...
				n = this.pointAttr[""],
				m, o, q, t = this.markerGroup,
				s = p(l.enabled, !this.requireSorting || this.activePointCount < 0.5 * this.xAxis.len / l.radius);
			if (l.enabled !== !1 || this._hasPointMarkers) for (f = b.length; f--;) if (g = b[f], d = U(g.plotX), e = g.plotY, k = g.graphic, m = g.marker || {}, o = !! g.marker, a = s && m.enabled === u || m.enabled, q = c.isInsidePlot(v(d), e, c.inverted), a && e !== u && !isNaN(e) && g.y !== null) if (a = g.pointAttr[g.selected ? "select" : ""] || n, h = a.r, i = p(m.symbol, this.symbol), j = i.indexOf("url") === 0, k) k[q ? "show" : "hide"](!0).animate(r({
				...
				if (q && (h > 0 || j)) g.graphic = c.renderer.symbol(i, d - h, e - h, 2 * h, 2 * h, o ? m : l).attr(a).add(t)
	...
	var qa = ma(O, {
		...
		getSegmentPath: function(a) {
			...
			if (e.stacking && !this.closedStacks) for (d = a.length - 1; d >= 0; d--) g = p(a[d].yBottom, f), d < a.length - 1 && e.step && c.push(a[d + 1].plotX, g), c.push(a[d].plotX, g);
	...
	ia = ma(O, {
		...
		getColumnMetrics: function() {
			...
			var c = L(Q(c.transA) * (c.ordinalSlope || b.pointRange || c.closestPointRange || c.tickInterval || 1), c.len),
				j = c * b.groupPadding,
				k = (c - 2 * j) / i,
				l = b.pointWidth,
		...
		translate: function() {
			...
			q(a.points, function(c) {
				var d = p(c.yBottom, f),
					h = L(t(-999 - d, c.plotY), e.len + 999 + d),
					q = c.plotX + k,
					r = j,
					s = L(h, d),
					u;
		...
		drawPoints: function() {
			...
			q(a.points, function(h) {
				var i = h.plotY,
					j = h.graphic;
				if (i !== u && !isNaN(i) && h.y !== null) f = h.shapeArgs, i = s(a.borderWidth) ? {
					"stroke-width": a.borderWidth
				} : {}, g = h.pointAttr[h.selected ? "select" : ""] || a.pointAttr[""], j ? (bb(j), j.attr(i)[b.pointCount < e ? "animate" : "attr"](w(f))) : h.graphic = d[h.shapeType](f).attr(g).attr(i).add(a.group).shadow(c.shadow, null, c.stacking && !c.borderRadius);
		...
		animate: function(a) {
			var b = this.yAxis,
				...
				e = {};
			if (ba) a ? (e.scaleY = 0.001, a = L(b.pos + b.len, t(b.pos, b.toPixels(c.threshold))), d ? e.translateX = a - b.len : e.translateY = a, this.group.attr(e)) : (e.scaleY = 1, e[d ? "translateX" : "translateY"] = b.pos, this.group.animate(e, this.options.animation), this.animate = null)
		},
		remove: function() {
			var a = this,
				b = a.chart;
	...
	T = {
		...
		translate: function(a) {
			...
			var b = 0,
				...
				j = this.startAngleRad = oa / 180 * (i - 90),
				i = (this.endAngleRad = oa / 180 * (p(c.endAngle, i + 360) - 90)) - j,
				k = this.points,
	...
	if (H.pie) H.pie.prototype.drawDataLabels = function() {
		...
		if (a.visible && (e.enabled || a._hasPointLabels)) {
				...
				if (K) {
					...
					if (n > 0) {
						...
						for (z = 0; z < K; z++) {
							...
							c = 9999;
							var S, P;
							for (P = 0; P < w; P++) S = Q(G[P] - x[1]), S < c && (c = S, E = P);
							if (E < z && G[z] !== null) E = z;
							else for (w < K - z + E && G[z] !== null && (E = w - K + z); G[E] === null;) E++;
							M.push({
								i: E,
								y: G[E]
							});
			...
			if (Ca(R) === 0 || this.verifyDataLabelOverflow(R)) this.placeDataLabels(), s && g && q(this.points, function(b) {
				j = b.connector;
				x = b.labelPos;
				if ((u = b.dataLabel) && u._pos) I = u._attr.visibility, y = u.connX, B = u.connY, k = l ? ["M", y + (x[6] === "left" ? 5 : -5), B, "C", y, B, 2 * x[2] - x[4], 2 * x[3] - x[5], x[2], x[3], "L", x[4], x[5]] : ["M", y + (x[6] === "left" ? 5 : -5), B, "L", x[2], x[3], "L", x[4], x[5]], j ? (j.animate({
					d: k
	...
	if (H.pie) H.pie.prototype.drawTracker = T.drawTrackerPoint;
...
(function(l, C) {
		...
		O = {
			...
			getPlotBandPath: function(a, b, c) {
				var d = this.center,
					...
					f = d[2] / 2,
					h = [o(c.outerRadius, "100%"), c.innerRadius, o(c.thickness, 10)],
					j = /%$/,
					k, m = this.isCircular;
	...
	r(G, "init", function(a, b, c) {
		...
		var m = c.pane || 0;
		if (d) {
			if (F(this, h ? V : O), j = !f) this.defaultRadialOptions = this.defaultRadialGaugeOptions
	...
	r(y, "getLabelPosition", function(a, b, c, d, e, f, h, j, k) {
		...
		m.isRadial ? (a = m.getPosition(this.pos, m.center[2] / 2 + o(f.distance, -25)), f.rotation === "auto" ? d.attr({
			rotation: g
		}) : i === null && (i = m.chart.renderer.fontMetrics(d.styles.fontSize).b - d.getBBox().height / 2), n === null && (n = m.isCircular ? g > 20 && g < 160 ? "left" : g > 200 && g < 340 ? "right" : "center" : "center", d.attr({
			align: n
		})), a.x += f.x, a.y += i) : a = a.call(this, b, c, d, e, f, h, j, k);
		return a
	});
	...
	z = {
		...
		translate: function() {
			...
			s(this.points, function(d) {
				...
				i && typeof i === "number" ? (i = i / 180 * Math.PI, n = Math.max(a.startAngleRad - i, Math.min(a.endAngleRad + i, n))) : b.wrap === !1 && (n = Math.max(a.startAngleRad, Math.min(a.endAngleRad, n)));
				n = n * 180 / Math.PI;
				d.shapeType = "path";
				d.shapeArgs = {
	...
	g.boxplot = v(g.column, {
		...
		drawPoints: function() {
			var a = this,
				...
				c = a.options,
				d = a.chart.renderer,
				e, f, h, j, k, m, i, n, g, l, p, I, r, q, J, u, v, t, w, x, z, y, E = a.doQuartiles !== !1,
			...
			s(b, function(b) {
				...
				u = {};
				y = b.color || a.color;
				if (b.plotY !== C) if (e = b.pointAttr[b.selected ? "selected" : ""], v = z.width, t = A(z.x), w = t + v, x = D(v / 2), f = A(E ? b.q1Plot : b.lowPlot), h = A(E ? b.q3Plot : b.lowPlot), j = A(b.highPlot), k = A(b.lowPlot), p.stroke = b.stemColor || c.stemColor || y, p["stroke-width"] = o(b.stemWidth, c.stemWidth, c.lineWidth), p.dashstyle = b.stemDashStyle || c.stemDashStyle, q.stroke = b.whiskerColor || c.whiskerColor || y, q["stroke-width"] = o(b.whiskerWidth, c.whiskerWidth, c.lineWidth), u.stroke = b.medianColor || c.medianColor || y, u["stroke-width"] = o(b.medianWidth, c.medianWidth, c.lineWidth), u["stroke-linecap"] = "round", i = p["stroke-width"] % 2 / 2, n = t + x + i, l = ["M", n, h, "L", n, j, "M", n, f, "L", n, k], E && (i = e["stroke-width"] % 2 / 2, n = A(n) + i, f = A(f) + i, h = A(h) + i, t += i, w += i, I = ["M", t, h, "L", t, f, "L", w, f, "L", w, h, "L", t, h, "z"]), B && (i = q["stroke-width"] % 2 / 2, j += i, k += i, r = ["M", n - x * B, j, "L", n + x * B, j, "M", n - x * B, k, "L", n + x * B, k]), i = u["stroke-width"] % 2 / 2, m = D(b.medianPlot) + i, J = ["M", t, m, "L", w, m], g) b.stem.animate({
					d: l
	...
	g.bubble = v(g.scatter, {
		...
		getRadii: function(a, b, c, d) {
			var e, f, h, j = this.zData,
				k = [],
				m = this.options.sizeBy !== "width";
			for (f = 0, e = j.length; f < e; f++) h = b - a, h = h > 0 ? (j[f] - a) / (b - a) : 0.5, m && h >= 0 && (h = Math.sqrt(h)), k.push(w.ceil(c + h * (d - c)) / 2);
			this.radii = k
		},
		animate: function(a) {
		...
		this.tickPositions && (s(this.series, function(b) {
			...
			if (b.bubblePadding && (b.visible || !c.options.chart.ignoreHiddenSeries)) if (a.allowZoomOutside = !0, p.push(b), f) s(["minSize", "maxSize"], function(a) {
				var b = h[a],
					...
					b = x(b);
				k[a] = f ? m * b / 100 : b
			}), b.minPxSize = k.minSize, b = b.zData, b.length && (i = o(h.zMin, w.min(i, w.max(P(b), h.displayNegative === !1 ? h.zThreshold : -Number.MAX_VALUE))), n = o(h.zMax, w.max(n, Q(b))))
		}), s(p, function(a) {
			var b = a[h],
				c = b.length,
				m;
	...
	(function() {
		...
		if (g.column) e = g.column.prototype, r(e, "animate", b), r(e, "translate", function(a) {
			...
			if (b.isRadial) {
				b = this.points;
				for (l = b.length; l--;) g = b[l], a = g.barX + e, g.shapeType = "path", g.shapeArgs = {
...
(function(d) {
	...
	function w(f, a, b, c, e, g, d, i) {
		var h = [];
		return g > e && g - e > n / 2 + 1.0E-4 ? (h = h.concat(w(f, a, b, c, e, e + n / 2, d, i)), h = h.concat(w(f, a, b, c, e + n / 2, g, d, i))) : g < e && e - g > n / 2 + 1.0E-4 ? (h = h.concat(w(f, a, b, c, e, e - n / 2, d, i)), h = h.concat(w(f, a, b, c, e - n / 2, g, d, i))) : (h = g - e, ["C", f + b * m(e) - b * B * h * l(e) + d, a + c * l(e) + c * B * h * m(e) + i, f + b * m(g) + b * B * h * l(g) + d, a + c * l(g) - c * B * h * m(g) + i, f + b * m(g) + d, a + c * l(g) + i])
	}
	function D(f) {
		if (this.chart.is3d()) {
	...
	d.SVGRenderer.prototype.cuboid = function(f) {
		var a = this.g(),
			f = this.cuboidPath(f);
		a.front = this.path(f[0]).attr({
			zIndex: f[3],
			"stroke-linejoin": "round"
		}).add(a);
		a.top = this.path(f[1]).attr({
			zIndex: f[4],
			"stroke-linejoin": "round"
		}).add(a);
		a.side = this.path(f[2]).attr({
			zIndex: f[5],
			"stroke-linejoin": "round"
		}).add(a);
		a.fillSetter = function(a) {
			var c = d.Color(a).brighten(0.1).get(),
				e = d.Color(a).brighten(-0.1).get();
			this.front.attr({
				fill: a
			});
			this.top.attr({
		...
		a.animate = function(a, c, e) {
			A(a.x) && A(a.y) ? (a = this.renderer.cuboidPath(a), this.front.attr({
				zIndex: a[3]
			}).animate({
				d: a[0]
			}, c, e), this.top.attr({
	...
	d.SVGRenderer.prototype.cuboidPath = function(d) {
		var a = d.x,
			...
			a = x(a, i, h, d.origin),
			d = ["M", a[0].x, a[0].y, "L", a[7].x, a[7].y, "L", a[6].x, a[6].y, "L", a[1].x, a[1].y, "Z"],
			b = ["M", a[3].x, a[3].y, "L", a[2].x, a[2].y, "L", a[5].x, a[5].y, "L", a[4].x, a[4].y, "Z"],
			c = ["M", a[1].x, a[1].y, "L", a[2].x, a[2].y, "L", a[5].x, a[5].y, "L", a[6].x, a[6].y, "Z"],
			e = ["M", a[0].x, a[0].y, "L", a[7].x, a[7].y, "L", a[4].x, a[4].y, "L", a[3].x, a[3].y, "Z"];
		return [["M", a[0].x, a[0].y, "L", a[1].x, a[1].y, "L", a[2].x, a[2].y, "L", a[3].x, a[3].y, "Z"], a[7].y < a[1].y ? d : a[4].y > a[2].y ? b : [], a[6].x > a[1].x ? c : a[7].x < a[0].x ? e : [], (a[0].z + a[1].z + a[2].z + a[3].z) / 4, h > 0 ? (a[0].z + a[7].z + a[6].z + a[1].z) / 4 : (a[3].z + a[2].z + a[5].z + a[4].z) / 4, i > 0 ? (a[1].z + a[2].z + a[5].z + a[6].z) / 4 : (a[0].z + a[7].z + a[4].z + a[3].z) / 4]
	};
	d.SVGRenderer.prototype.arc3d = function(f) {
		...
		var a = this.g(),
			...
			e = b.zTop * 100;
		a.shapeArgs = f;
		a.top = c.path(b.top).attr({
			zIndex: b.zTop
		}).add(a);
		a.side1 = c.path(b.side2).attr({
			zIndex: b.zSide2
		});
		a.side2 = c.path(b.side1).attr({
			zIndex: b.zSide1
		});
		a.inn = c.path(b.inn).attr({
			zIndex: b.zInn
		});
		...
		a.animate = function(a, b, c) {
			A(a.end) || A(a.start) ? (this._shapeArgs = this.shapeArgs, d.SVGElement.prototype.animate.call(this, {
				_args: a
			}, {
				duration: b,
				step: function() {
	...
	d.SVGRenderer.prototype.arc3dPath = function(d) {
		...
		var v = i * l(k),
			u = i * l(h),
			i = ["M", a + r * o, b + s * q],
			i = i.concat(w(a, b, r, s, c, e, 0, 0)),
			i = i.concat(["L", a + p * d, b + j * y]),
			i = i.concat(w(a, b, p, j, e, c, 0, 0)),
			i = i.concat(["Z"]),
			k = k > 0 ? n / 2 : 0,
			h = h > 0 ? 0 : n / 2,
			k = c > -k ? c : e > -k ? -k : c,
			t = e < n - h ? e : c < n - h ? n - h : e,
			h = ["M", a + r * m(k), b + s * l(k)],
			h = h.concat(w(a, b, r, s, k, t, 0, 0)),
			h = h.concat(["L", a + r * m(t) + v, b + s * l(t) + u]),
			h = h.concat(w(a, b, r, s, t, k, v, u)),
			h = h.concat(["Z"]),
			k = ["M", a + p * o, b + j * q],
			k = k.concat(w(a, b, p, j, c, e, 0, 0)),
			k = k.concat(["L", a + p * m(e) + v, b + j * l(e) + u]),
			k = k.concat(w(a, b, p, j, e, c, v, u)),
			k = k.concat(["Z"]),
			o = ["M", a + r * o, b + s * q, "L", a + r * o + v, b + s * q + u, "L", a + p * o + v, b + j * q + u, "L", a + p * o, b + j * q, "Z"],
			a = ["M", a + r * d, b + s * y, "L", a + r * d + v, b + s * y + u, "L", a + p * d + v, b + j * y + u, "L", a + p * d, b + j * y, "Z"],
			b = l((c + e) / 2),
			c = l(c),
			e = l(e);
		return {
	...
	d.Chart.prototype.retrieveStacks = function(f, a) {
		...
		d.each(this.series, function(a) {
			b[a.options.stack || 0] ? b[a.options.stack || 0].series.push(a) : (b[a.options.stack || 0] = {
				series: [a],
				position: c
			}, c++)
		});
	...
	d.wrap(d.Axis.prototype, "render", function(d) {
		...
		if (this.chart.is3d()) {
			var a = this.chart,
				...
				e = c.alpha,
				g = c.beta * (a.yAxis[0].opposite ? -1 : 1),
				j = c.frame,
				i = j.bottom,
				h = j.back,
				j = j.side,
	...
	d.wrap(d.Axis.prototype, "getPlotLinePath", function(d) {
		...
		var a = [{
			x: a[1],
			y: a[2],
			z: this.horiz || this.opposite ? e : 0
		}, {
			x: a[1],
			y: a[2],
			z: e
			...
			e = b.options.inverted ? c.beta : c.alpha,
			g = b.options.inverted ? c.alpha : c.beta;
	...
	d.wrap(d.Tick.prototype, "getMarkPath", function(d) {
		...
		var b = this.axis.chart,
			...
			g = b.inverted ? c.beta : c.alpha,
			c = b.inverted ? c.alpha : c.beta;
	...
	d.wrap(d.seriesTypes.column.prototype, "translate", function(f) {
		...
		if (this.chart.is3d()) {
			var a = this.chart,
				...
				j = c.alpha,
				i = c.beta * (a.yAxis[0].opposite ? -1 : 1),
				h = (b.stacking ? b.stack || 0 : this._i) * (e + (b.groupZPadding || 1));
			b.grouping !== !1 && (h = 0);
			h += b.groupZPadding || 1;
			d.each(this.data, function(a) {
				if (a.y !== null) {
	...
	d.wrap(d.seriesTypes.column.prototype, "animate", function(f) {
		if (this.chart.is3d()) {
			var a = arguments[1],
				b = this.yAxis,
				c = this,
	...
	C = d.extendClass(d.seriesTypes.column, {
		type: "cylinder"
	});
	...
	d.wrap(d.seriesTypes.cylinder.prototype, "translate", function(f) {
		...
		if (this.chart.is3d()) {
			var a = this.chart,
				...
				j = b.alpha,
				i = c.stacking ? (this.options.stack || 0) * e : this._i * e;
			i += e / 2;
			c.grouping !== !1 && (i = 0);
			d.each(this.data, function(a) {
				var b = a.shapeArgs;
	...
	d.wrap(d.seriesTypes.pie.prototype, "translate", function(f) {
		...
		if (this.chart.is3d()) {
			var a = this,
				...
				i = g.alpha,
				h = g.beta,
				k = c.stacking ? (c.stack || 0) * e : a._i * e;
			k += e / 2;
			c.grouping !== !1 && (k = 0);
	...
	g.extend(n.prototype, {
		...
		parseTypes: function() {
			for (var a = this.columns, b = this.rawColumns, c = a.length, f, e, d, i, g, h, j = [], l, m = this.chartOptions; c--;) {
				f = a[c].length;
				b[c] = [];
				for (l = (g = q(c, this.valueCount.xColumns) !== -1) && m && m.xAxis && s(m.xAxis)[0].type === "category"; f--;) if (e = j[f] || a[c][f], d = parseFloat(e), i = b[c][f] = this.trim(e), l) a[c][f] = i;
				else if (i == d) a[c][f] = d, d > 31536E6 ? a[c].isDatetime = !0 : a[c].isNumeric = !0;
		...
		complete: function() {
			...
			if (c.complete || c.afterComplete) {
				...
				for (d = 0; d < this.valueCount.seriesBuilders.length; d++) h = this.valueCount.seriesBuilders[d], h.populateColumns(e) && g.push(h);
				for (; e.length > 0;) {
					h = new l;
				...
				if (b === "category") for (d = 0; d < g.length; d++) {
					h = g[d];
					for (e = 0; e < h.readers.length; e++) if (h.readers[e].configName === "x") h.readers[e].configName = "name"
				}
	...
	g.wrap(g.Chart.prototype, "init", function(a, b, c) {
		...
		b && b.data ? g.data(g.extend(b.data, {
			afterComplete: function(e) {
				var d, i;
				if (b.hasOwnProperty("series")) if (typeof b.series === "object") for (d = Math.max(b.series.length, e.series.length); d--;) i = b.series[d] || {}, b.series[d] = g.merge(i, e.series[d]);
