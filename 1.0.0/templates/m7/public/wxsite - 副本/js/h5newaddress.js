		...
		if(addresslist.length > 0){
			...
			$.each(addresslist, function(i, newobj) { 
				if(newobj.default ==1){
					...
					var addhtmls = '<div class="wmr_subord_head_add">'
					              +'<span>'+completeaddress+'</span><b></b></div>'
								  +'<div class="wmr_subord_head_tel"><span>'+newobj.contactname+' '+newobj.phone+'</span></div>'								  
								  +'<input type="hidden" id="addresslng" value="'+newobj.lng+'" />'
								  +'<input type="hidden" id="addresslat" value="'+newobj.lat+'" />';						  
					$('#orderaddress').html(addhtmls);
				}		
			}); 		 

		}else{			 
			var nahtmls = '<div class="wmr_subord_head_add">'			
		$('#orderaddress').html('<div class="wmr_subord_head_add" style="height:50px"><div class="noaddress">+添加收货地址</div> </div>');
		}
	}else{
	    newTmsg(bk.content);
	}
...
function checkPsrangePscost(newobj){
	...
	nopsparghtml = '';
	 ...
	 function baitchangeaddress2(){	 
		$(".wmr_title_center").text('管理地址'); 
		$('.wmr_subord_head_box').hide();
		$("#gdcart").hide("slow");
