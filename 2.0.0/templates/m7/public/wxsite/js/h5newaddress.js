		...
		if(addresslist.length > 0){
			...
			$.each(addresslist, function(i, newobj) { 
				if(newobj.default ==1){
					...
					var addhtmls = '<div class="fb_subord_head_add">'
					              +'<span>'+completeaddress+'</span><b></b></div>'
								  +'<div class="fb_subord_head_tel"><span>'+newobj.contactname+' '+newobj.phone+'</span></div>'								  
			...
			var nahtmls = '<div class="fb_subord_head_add">'			
		$('#orderaddress').html('<div class="fb_subord_head_add" style="height:50px"><div class="noaddress">+添加收货地址</div> </div>');
		}
	}else{
	    newTmsg(bk.content);
	}
...
function checkPsrangePscost(newobj){
	...
	nopsparghtml = '';
	 ...
	 function baitchangeaddress2(){	 
		$(".fb_title_center").text('管理地址'); 
		$('.fb_subord_head_box').hide();
		$("#gdcart").hide("slow");
