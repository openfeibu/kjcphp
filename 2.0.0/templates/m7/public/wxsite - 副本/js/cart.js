	  ...
	  if(datas.flag == false){
		...
		/*==================追加打包费信息=================*/ 
		 if(datas.content.bagcost != 0){
       	  	  temp_htmls = '<li><div class="fb_subord_info_l">餐盒费</div><div class="fb_subord_info_c"></div><div class="fb_subord_info_r">&yen;'+datas.content.bagcost+'</div></li>';
			  $('#foodslist').append(temp_htmls); 
		 }
		 /*==================追加配送费信息===================*/ 
		if(datas.content.pscost != 0){
			  $('input[name="pscost"]').val(datas.content.pscost);
       	  	  temp_htmls = '<li><div class="fb_subord_info_l">配送费</div><div class="fb_subord_info_c"></div><div class="fb_subord_info_r">&yen;'+datas.content.pscost+'</div></li>';
			  $('#foodslist').append(temp_htmls); 
		 }
		 /*=================追加附加配送费信息=================*/ 
       if(morepscost>0){ 
			  $('input[name="addpscost"]').val(morepscost);
       	  	  temp_htmls = '<li><div class="fb_subord_info_l">附加配送费</div><div class="fb_subord_info_c"></div><div class="fb_subord_info_r">&yen;'+morepscost+'</div></li>';
			  $('#foodslist').append(temp_htmls); 
		 }
		 /*==============遍历追加优惠活动信息===============*/ 
	...
	if(checknext ==  false){ 
	  ...
	  if(jifenbili > 0 && rslt > 1){ 
	      for(var i=1;i<=cancost;i++){	      	 
			  ...
			  jfhtml += '<li class="jflist jf_'+i+'"  onclick="selectjifen(\''+i+'\',\''+jifenall+'\',\'使用'+jifenall+'抵扣'+i+'元\');"><div class="fb_subord_popup_l">使用'+jifenall+'积分</div><div class="fb_subord_popup_c">抵扣'+i+'元</div><div class="fb_subord_popup_r"><input type="radio" /></div></li>';		      	
	      }
		  var cjf = $('#jfcost').attr('data'); 
		  if(cjf != 1){
			 $('.fb_jfsubord_popup_list ul').append(jfhtml);	
		     $('.fb_jfsubord_popup_list ul').append('<div style="position:fixed;bottom:0;width:100%;" onclick="notusejf()" class="fb_subord_popup_tit notusejf">不使用积分抵扣</div>'); 
		  }
...
function notusejf(){
		$('.fb_subord_jf').html('<h3>积分抵扣</h3><span>不使用积分<i class="fa fa-angle-right"></i></span>');     
		$('.jfmaskbg').hide(); 
		$('#jfcost').attr('data',1);
...
function selectjifen(dikoujin,dikoujf,names){
	...
	if(checknext ==  false){ 
	   ...
	   $('.fb_subord_jf').html('<h3>积分抵扣</h3><span style="color:#ff6e6e">-￥'+dikoujin+'<i class="fa fa-angle-right"></i></span>');
       $('#jfcost').attr('data',1);
	   $('.jflist').removeClass('navaA'); 
...
function maketime(){
	if(timelist.length > 0){
		...
		if(timelist[0].cost > 0){
			$('.fb_subord_top h3').html(timelist[0].name+'(需附加配送费￥<font  color=red  id="morepscost">'+timelist[0].cost+'</font>元)');
...
function gettimelist(){	
	...
	$.each(timelist,function(i,field){
		 ...
		 tmhtml += '<li class="" id="'+field.value+'" onclick="selectTime(\''+field.value+'\',\''+field.name+'\',\''+field.cost+'\');"><div class="fb_subord_popup_l">'+field.name+'</div><div class="fb_subord_popup_c">'+pscost+'元配送费</div><div class="fb_subord_popup_r"><input type="radio" /></div></li>'				  
		 $('.fb_subord_popup_list ul').append(tmhtml);		 
    });
	$('.fb_subord_popup_list ul').append('<div style="position:fixed;bottom:0;width:100%;" class="fb_subord_popup_tit">取消</div>');
}
/*==================点击选择配送时间====================*/
...
function selectTime(timename,timetitle,cost){ 
 ...
 if(cost > 0){
	 $('.fb_subord_top h3').html(timetitle+'(需附加配送费￥<font  color=red  id="morepscost">'+cost+'</font>元)');  
	 $('input[name="addpscost"]').val(cost);
 }else{
	$('.fb_subord_top h3').html(timetitle);
 }
 $('.fb_subord_popup_list ul li').removeClass('navaA'); 
...
function selectjuan(juanid,juancost,limitcost,juanname,juanpaytype){
	...
	$('#orderaddress').show();
	$('.fb_title_center').text('订单配送至');
	$('.fb_subord_yh').html('<h3>代金券</h3><span style="color:#ff6e6e">-￥'+juancost+'<i class="fa fa-angle-right"></i></span>');
...
function notusejuan(){
	$('#orderaddress').show();
		$('.fb_title_center').text('订单配送至');
		$('.fb_subord_yh').html('<h3>代金券</h3><span>不使用代金券<i class="fa fa-angle-right"></i></span>');     
		$('#mask1').hide();
		$('#popup1').hide();
		$('#gdcart').show();  
		$('#yhjlist').hide();
...
function getjuaninfo(){
	...
	if(juanlist.length > 0){	
		...
		if(juancount > 0){
			$('.fb_subord_yh').html('<h3>代金券</h3><span style="color:#ff6e6e">'+juancount+'张可用<i class="fa fa-angle-right"></i></span>');
			$('#yhjlist span i').text(juancount);        			 			  
		}else{
			 $('.fb_subord_yh').html('<h3>代金券</h3><span>暂无可用<i class="fa fa-angle-right"></i></span>');  
