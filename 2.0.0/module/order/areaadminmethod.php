...
class method extends areaadminbaseclass
{
    ...
    public function ordertoday()
    {
        ...
        $this->setstatus();
        $nowday = date('Y-m-d', time());
        $where = '  where  ord.admin_id = '.$this->admin['cityid'].' and  ord.addtime > '.strtotime($nowday.' 00:00:00').' and ord.addtime < '.strtotime($nowday.' 23:59:59');
        //查询当天所有订单数据
        ...
        $areainfo = $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."area where admin_id =  '".$this->admin['cityid']."'  order by orderid asc");
         ...
         case 'pass':
          ...
          case 'send':
                ...
                if ($orderinfo['status'] != 1) {
                  ...
                  if ($orderinfo['is_make'] == 0) {
                      $this->message('等待商家确认制作后才能发货');
                  }
                  if ($orderinfo['is_make'] == 2) {
                      $this->message('商家不制作该订单不能发货');
                  }
                  if ($orderinfo['pstype'] == 1) {
                      $this->message('商品发货订单请通过商家管理发货');
                  }
                if ($orderinfo['is_goshop']  !=1) {
                    if ($orderinfo['pstype'] == 2) {
                        $this->message('第三方取货后自动发货');
                    }
                }
          ...
          break;
           case 'psyuan':
                if ($orderinfo['is_goshop'] == 1) {
                    $this->message('到店买单订单不需要配送');
                }
                if ($orderinfo['status'] > 2) {
                    $this->message('order_baklogcantdoover');
                }
                if (!empty($orderinfo['psuid'])) {
                    $this->message('order_setpsyuan');
                }
                if ($orderinfo['pstype'] == 2) {
                    $this->message('第三方分配订单');
                }
    ...
    public function ajaxcheckorder()
    {
        ...
        $shuliang1  = $this->mysql->counts("select ord.* from ".Mysite::$app->config['tablepre']."order as ord   ".$where." and ord.admin_id = ".$this->admin['cityid']."  and ord.is_make = 0 ");
    ...
    public function outtjorder()
    {
        ...
        if (is_array($shoplist)) {
            foreach ($shoplist as $key=>$value) {
                ...
                $shoptj=  $this->mysql->select_one("select  count(id) as shuliang,sum(allcost) as allcost,sum(cxcost) as cxcost,sum(yhjcost) as yhcost, sum(shopcost) as shopcost,sum(scoredown) as score, sum(shopps)as pscost, sum(bagcost) as bagcost from ".Mysite::$app->config['tablepre']."order  where shopid = '".$value['id']."' and paytype =0 and shopcost > 0 and status = 3 ".$where2." order by id asc  limit 0,1000");
    ...
    ***************/
    ...
    public function shopjsover()
    {
        ...
        $shuliang  = $this->mysql->counts("select *  from ".Mysite::$app->config['tablepre']."shopjs ".$where."  order by id asc  ");
        $pageshow->setnum($shuliang);
        $data['pagecontent'] = $pageshow->getpagebar();
        $tempdata = array();
        if (is_array($txlist)) {
