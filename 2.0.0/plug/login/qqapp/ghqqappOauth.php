		...
		if($userinfo['ret']  == 0){
			//登录成功  
			 ...
			 if(empty($oauthqqinfo)){
					...
					$data['userlogo'] = $userinfo['figureurl_2'];  
					*/
				    $tempuid = 0;
				    $data['uid'] = $tempuid;
					...
					$data['type'] = 'qqapp';
					$data['addtime'] = time();
					//$data['logo'] = $userinfo['figureurl_2'];  
					$this->mysql->insert(Mysite::$app->config['tablepre'].'oauth',$data);  
					$oauthqqinfo = $data;
					 
			}
			$this->oauthinfo = array('phone'=>'','uid'=>0);
			if(!empty($userinfo['figureurl_2'])){ 
				$this->oauthinfo['logo'] =$userinfo['figureurl_2'];
			}else{
			...
			if($oauthqqinfo['uid'] == 0){
				...
				if(!empty($checkuser)&& !empty($checkuser['phone'])){
					...
					if(!empty($this->oauthinfo['figureurl_2'])){ 
						$this->oauthinfo['logo'] = preg_match('/(http:\/\/)|(https:\/\/)/i',$this->oauthinfo['figureurl_2'])?$this->oauthinfo['figureurl_2']:Mysite::$app->config['siteurl'].$this->oauthinfo['logo'];
					}else{
						$this->oauthinfo['logo'] = Mysite::$app->config['siteurl'].Mysite::$app->config['userlogo'];
					}
