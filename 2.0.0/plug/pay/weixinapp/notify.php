				...
				$uid = $checked[1];
				  ...
				  $checkinfo =mysql_query("SELECT * from `".$cfg['tablepre']."onlinelog` where dno = '".$result['transaction_id']."' "); 
				   ...
				   if(empty($checkinfo['status'])){
						...
						mysql_query("INSERT INTO `".$cfg['tablepre']."memcostlog`(`uid`,`username`,`cost`,`bdtype`,`bdcost`,`curcost`,`bdliyou`,`czuid`,`czusername`,`addtime`) VALUES('".$memberinfo['uid']."','".$memberinfo['username']."','".$memberinfo['cost']."',1,'".$acountcost."','".$checkcost."','微信app充值','".$memberinfo['uid']."','".$memberinfo['username']."','".$dotime."');");
