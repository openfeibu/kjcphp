...
class upload
{
    ...
    public $fileFormat = array('gif', 'jpg', 'png', 'application/octet-stream'); // 文件格式&MIME限定
    public $overwrite = 0; // 覆盖模式
    public $maxSize = 1048576; // 文件最大字节
    public $ext; // 文件扩展名
    public $thumb = 1; // 是否生成缩略图
    public $thumbWidth = 40; // 缩略图宽
    public $thumbHeight = 40; // 缩略图高
    ...
    public function upload($savePath, $fileFormat = '', $thumb=1, $maxSize = 0, $overwrite = 0, $fileInput = 'imgFile', $changeName=1)
    {
        $info = webCfg::init();
    ...
    public function upload_file($url, $filename, $path, $type, $input_name)
    {
        try {
            /* php 5.5及以上
            $ch = curl_init($url);
    ...
    public function setThumb($thumb, $thumbPrefix, $thumbWidth = 0, $thumbHeight = 0)
    {
        $this->thumb = $thumb;
    ...
    public function imageWaterMark($groundImage, $waterPos=0, $waterImage="", $waterText="", $fontSize=12, $textColor="#CCCCCC", $fontfile='font.ttf', $xOffset=0, $yOffset=0)
    {
        global $config;
