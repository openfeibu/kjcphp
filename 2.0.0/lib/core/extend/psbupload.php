...
class upload
{
    ...
    function upload($configdata,$fileFormat = '',$thumb=0, $maxSize = 0, $overwrite = 0 ,  $changeName=1){ 
		$this->setConfig($configdata);
		$this->thumb = 0;
        $this->setSavepath($this->savePath);
		$this->makeDirectory(hopedir.$this->savePath);//创建上传目录
        $this->setFileformat($fileFormat);
        $this->setMaxsize($maxSize);
        $this->setOverwrite($overwrite);
        $this->setThumb($this->thumb, $this->thumbWidth, $this->thumbHeight);
        $this->errno = 0; 
		$this->run($this->fileInput,$changeName);
    }
	
	
	function setConfig($data){
		$this->Siteconfig = array('img_max_size'=>8,
		'img_large_height'=>400,
		'img_large_width'=>400,
		'img_middle_height'=>150,
		'img_middle_width'=>150,
		'img_small_height'=>80,
		'img_small_width'=>80, 
		); 
		if(isset($_POST['img_large_height'])){
			 $this->Siteconfig['img_large_height'] = intval($_POST['img_large_height']);
		}
		if(isset($_POST['img_large_width'])){
			 $this->Siteconfig['img_large_width'] = intval($_POST['img_large_width']);
		}
		if(isset($_POST['img_middle_height'])){
			 $this->Siteconfig['img_middle_height'] = intval($_POST['img_middle_height']);
		}
		if(isset($_POST['img_middle_width'])){
			 $this->Siteconfig['img_middle_width'] = intval($_POST['img_middle_width']);
		}
		if(isset($_POST['img_small_height'])){
			 $this->Siteconfig['img_small_height'] = intval($_POST['img_small_height']);
		}
		if(isset($_POST['img_small_width'])){
			 $this->Siteconfig['img_small_width'] = intval($_POST['img_small_width']);
		} 
		if(isset($_POST['input_name'])){ 
			$this->fileInput = trim($_POST['input_name']);
		}
 		$imgdir = 'upload/psbimg/';
	    $this->savePath = $imgdir;
    ...
    function copyfile($fileArray)
    {
        ...
        // 如果有大小限制，检查文件是否超过限制
		if(!isset($this->Siteconfig['img_max_size'])){
			$this->errno = 23;
			return false;
		}
		$checksieze = $this->Siteconfig['img_max_size']*1024*1024;
		if($fileArray["size"] > $checksieze){
			 $this->errno = 14;
             return false;
		} 
		if(!isset($this->Siteconfig['img_large_height'])){
			$this->errno = 24;
			return false;
		}
		if(!isset($this->Siteconfig['img_large_width'])){
			$this->errno = 25;
			return false;
		}
		if(!isset($this->Siteconfig['img_middle_height'])){
			$this->errno = 26;
			return false;
		}
		if(!isset($this->Siteconfig['img_middle_width'])){
			$this->errno = 27;
			return false;
		}
		if(!isset($this->Siteconfig['img_small_height'])){
			$this->errno = 28;
			return false;
		}
		if(!isset($this->Siteconfig['img_small_width'])){
			$this->errno = 29;
			return false;
		} 
        ...
        if (!move_uploaded_file($fileArray["tmp_name"], $this->savePath . $this->saveName)) {
            ...
            if (strtolower($CreateFunction) == "imagecreatefromgif" && !function_exists("imagecreatefromgif")) {
                ...
                $this->errno = 17;
                return false;
            }elseif (strtolower($CreateFunction) == "imagecreatefrompng" && !function_exists("imagecreatefrompng")) {
                $this->errno = 17;
                return false;
            } elseif (!function_exists($CreateFunction)) {//imageCreateFromPng
                $this->errno = 18;
                return false;
            }
			...
			for($c = 0;$c< 3;$c++){
			    if($c == 0){
					$this->thumbPrefix = 'large'; 
					$this->thumbWidth = $this->Siteconfig['img_large_width']; 
					$this->thumbHeight = $this->Siteconfig['img_large_height']; 
				}elseif($c==1){
					$this->thumbPrefix = 'middle'; 
					$this->thumbWidth = $this->Siteconfig['img_middle_width']; 
					$this->thumbHeight = $this->Siteconfig['img_middle_height']; 
				}else{
					$this->thumbPrefix = 'small'; 
					$this->thumbWidth = $this->Siteconfig['img_small_width']; 
					$this->thumbHeight = $this->Siteconfig['img_small_height']; 
				}
				if (($originalHeight < $this->thumbHeight && $originalWidth < $this->thumbWidth)) {
					...
					if($originalWidth > $originalHeight){//图片宽度大于高度
					...
					*/
					...
					imagecolortransparent($createdThumb,$color); 
